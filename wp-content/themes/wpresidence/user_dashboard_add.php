...
if( isset( $_GET['listing_edit'] ) && is_numeric( $_GET['listing_edit'] ) ){
    ...
    /////// WEBSITING
    ///////////////////////////////////////////////////////////////////////////////////////////
    $prop_espace_array     =   get_the_terms($edit_id, 'property_espace_ext');
    if(isset($prop_espace_array[0])){
        $prop_espace_selected           =   $prop_espace_array[0]->term_id;
    }
    ///////////////////////////////////////////////////////////////////////////////////////////
    /////// END WEBSITING
    ///////////////////////////////////////////////////////////////////////////////////////////
...
if( 'POST' == $_SERVER['REQUEST_METHOD'] && $_POST['action']=='view' ) {
    ...
    if ( $paid_submission_status!='membership' || ( $paid_submission_status== 'membership' || wpestate_get_current_user_listings($userID) > 0)  ){ // if user can submit
        ...
        if( !isset($_POST['prop_espace']) ) {
            $prop_espace=0;           
        }else{
            $prop_espace  =   wp_kses($_POST['prop_espace'],$allowed_html);
        }
    ...
    //    $property_state                 =   wp_kses( $_POST['property_state'],$allowed_html);
        ...
        $property_latitude              =   wp_kses( $_POST['property_latitude'],$allowed_html); 
        $property_longitude             =   wp_kses( $_POST['property_longitude'],$allowed_html); 
        ...
        */
        ...
        if($post_id) {
            ...
            if ( isset ($prop_espace->name) ){
                wp_set_object_terms($post_id,$prop_espace->name,'property_espace_ext'); 
            }  
...
if( 'POST' == $_SERVER['REQUEST_METHOD'] && $_POST['action']=='edit' ) {
     if ( !isset($_POST['new_estate']) || !wp_verify_nonce($_POST['new_estate'],'submit_new_estate') ){
       exit('Sorry, your not submiting from site');
        ...
        if($prop_espace==-1){
            wp_delete_object_term_relationships($edit_id,'property_espace_ext'); 
        }    
...
$user_small_picture_id      =   get_the_author_meta( 'small_custom_picture' , $userID  );
if( $user_small_picture_id == '' ){

    $user_small_picture[0]=get_template_directory_uri().'/img/default_user_small.png';
}else{
    $user_small_picture=wp_get_attachment_image_src($user_small_picture_id,'full');
        ...
        <tr>
            ...
            <td class="content">
                ...
                <?php while (have_posts()) : the_post(); ?>
                   ...
                   <div class="dashboard_submit_top row">
                       <div class="col-md-6">
                           <div class="menu_user_picture user_picture_dashboard" style="background-image: url('<?php print $user_small_picture[0]; ?>');"></div>
                           <div class="user_name_dashboard">
                               <?php _e('Bonjour,','wpestate');?><br>
                               <?php print $first_name.' '.$last_name; ?>
                           </div>
                       </div>
                       <div class="col-md-6 border_left">
                           <div class="message_top_dashboard">
                               <?php _e('Déposez dès maintenant gratuitement','wpestate');?><br>
                               <span class="pad_left"><?php _e('et en quelques clics votre annonce !','wpestate');?></span>
                           </div>
                       </div>
                   </div>
