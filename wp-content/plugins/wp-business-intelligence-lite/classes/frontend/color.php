...
$min = 0;
	...
	switch($n%3){
		case 0:	$r = dechex(abs($max - ($n*50))); 
				$g = dechex(abs($min + ($n*50)));
				$b = dechex(abs($min + ($n*50)));
				break;
		case 1:	$r = dechex(abs($min + ($n*50)));
				$g = dechex(abs($max - ($n*50)));
				$b = dechex(abs($min + ($n*50)));
				break;
		case 2:	$r = dechex(abs($min + ($n*50)));
				$g = dechex(abs($min + ($n*50)));
				$b = dechex(abs($max - ($n*50)));
				break;
	}
	...
	$zero_b = (strlen($b)==1) ? '0' : '';
   	...
   	return ('#'.substr($zero_r.$r.$zero_g.$g.$zero_b.$b,0,6));
}
